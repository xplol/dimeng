...
滴萌监控注册与心跳 API 已通过 `https://xlx.wipecell.top` 接通。真实测试已验证：安装后显示出口 IP、一次性绑定码和主机指纹；登录用户可以认领服务器；其他用户不能读取该服务器、详情或指标。`v0.2.0` 新增终端绑定二维码，`v0.3.0` 增加结构化 doctor、能力上报和独立签名升级器，`v0.3.1` 修复签名升级器首次安装的目录初始化和失败清理，`v0.3.2` 确保升级监听单元在安装后立即启动，最终发布状态以 GitHub Release 为准。
...
发布 `v0.3.2` 后启用签名升级器时，需要同时设置 `DIMENG_VERSION=v0.3.2` 和 `DIMENG_ENABLE_SIGNED_UPGRADE=1`。发布辅助脚本 `scripts/build-release.sh` 会生成 Linux `amd64/arm64` Agent、升级器、SHA-256、manifest 和 Ed25519 签名；签名私钥不进入仓库。
...
- `v0.3.2` 支持自动接收升级建议，但只有固定公钥验签、SHA-256、版本、架构和下载域名白名单全部通过后才会替换二进制；可在 `/etc/dimeng-monitor-agent/agent.env` 设置 `DIMENG_AUTO_UPGRADE=false` 关闭。当前稳定安装入口仍是 `v0.2.0`，升级器必须随 `v0.3.2` 签名发布物一起显式启用。
